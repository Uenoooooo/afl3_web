            ...
            <tbody class="text-black">
                @forelse ($reminders as $index => $reminder)
                    <tr>
                        ...
                        <td class="border px-4 py-2">{{ $reminder->category->name ?? 'Tidak Ada Kategori' }}</td>
                        <td class="border px-4 py-2">{{ $reminder->payment_method->name ?? 'Tidak Ada Metode Pembayaran' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="10" class="border px-4 py-2 text-center">Tidak ada data pengingat.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
