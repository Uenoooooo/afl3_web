                ...
                <table class="table-auto w-full">
                    <thead class="bg-gray-100 text-gray-700">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-left">Reminder Name</th>
                    ...
                    <tbody>
                        @foreach ($reminders as $index => $reminder)
                            <tr class="border-t">
                                ...
                                <td class="px-4 py-2">{{ $reminder->reminder_name }}</td>
                                <td class="px-4 py-2">{{ $reminder->reminder_desc }}</td>
                                <td class="px-4 py-2">{{ $reminder->type->name ?? '-' }}</td>
                                <td class="px-4 py-2">Rp {{ number_format($reminder->reminder_amount, 0, ',', '.') }}</td>
                                <td class="px-4 py-2">{{ $reminder->start_date }}</td>
                                <td class="px-4 py-2">{{ $reminder->frequency->name ?? '-' }}</td>
                                <td class="px-4 py-2">{{ $reminder->category->name ?? '-' }}</td>
                                <td class="px-4 py-2">{{ $reminder->paymentMethod->name ?? '-' }}</td>
